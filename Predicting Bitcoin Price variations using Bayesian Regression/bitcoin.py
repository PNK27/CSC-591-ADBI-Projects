...
def similarity(a,b):
    ...
    for i in range(0,len(a)):
        nr = nr + ((a.iloc[i] - mean_B)*(b.iloc[i] - mean_B))
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
        y = Xi.iloc[i]
        exponential = np.exp(wt*similarity(x, (y.iloc[0:(len(y)-1)])))
        num = num + y[-1]*exponential
        den = den + exponential
...
# YOUR CODE HERE
formula = "deltaP ~ deltaP90 + deltaP180 + deltaP360"
model = smf.ols(formula, trainData).fit()
...
MSE = sm.mean_squared_error(compare['Actual'], compare['Predicted'])
