...
def simple(g):
	...
	# YOUR CODE HERE
	edges = g.edges
	flip_edges_df = edges.withColumn('temp',edges.dst)
	flip_edges_df = flip_edges_df.withColumn('dst',flip_edges_df.src)
	flip_edges_df = flip_edges_df.withColumn('src',flip_edges_df.temp)
	flip_edges_df = flip_edges_df.drop('temp')
	...
	edges = g.edges.unionAll(flip_edges_df)
	edges = edges.dropDuplicates()
	edges = edges.filter((edges.src > edges.dst) | (edges.src < edges.dst))
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	# graphframe vertices
	# YOUR CODE HERE
	vertices = lines.flatMap(lambda x:x.split(delim)).distinct()
	vertices = [(vertex,) for vertex in vertices.collect()]
	v = sqlContext.createDataFrame(vertices,['id'])
	# Create graphframe g from the vertices and edges.
	g = GraphFrame(v,e)
	return g

def powerLaw(distrib):
    # Install powerlaw package before running the script
    counts = distrib.map(lambda x: x[1]).collect()    
