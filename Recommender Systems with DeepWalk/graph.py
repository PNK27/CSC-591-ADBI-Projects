...
def records_to_graph():
    ...
    for m in movies:
        ...
        for r in ratings_enum:
            mn = Node(id, m+"_"+r, "movie-rating")
            nodedict[m+"_"+r] = mn
            nodelist.append(mn)
            id = id+1
    ...
    for single_rating in ratings:
        temp = ratings[single_rating]
        unode = nodedict[single_rating]
        
        for m in temp:
            rnode = m+"_"+temp[m]
            movie_rating_node = nodedict[rnode]
            unode.neighbors.append(movie_rating_node)
            movie_rating_node.neighbors.append(unode)  
    
    for m in movies:
        demo = movies[m]
        movie_node = nodedict[m]
        
        #creating edges between ratings and movies
        for r in ratings_enum:
            r_n = m+"_"+r
            rating_node = nodedict[r_n]
            rating_node.neighbors.append(movie_node)
            movie_node.neighbors.append(rating_node)
        
        #creating edges between genres and movies
        movie_gen = demo.genres    
        for m in movie_gen:
            genre_node = nodedict[g]
            genre_node.neighbors.append(movie_node)
            movie_node.neighbors.append(genre_node)
            
        #creating edges between actors and movies
        actors_mov = demo.actors
        for a in actors_mov:
            actor_node = nodedict[a]
            actor_node.neighbors.append(movie_node)
            movie_node.neighbors.append(actor_node)
        
        #creating edges between directors and movies
        if(demo.director!=None):
            director_node = nodedict[demo.director]
